...
Whats New:
	...
	1. Added the RTOS-DEMo and RTOS-CLI demo programs. These are not part of the kernel but seperate programs.
	2. modified some of the spi flash command implmentations
	3. modified the compiler swtiches see below for explanantions
	4. modified Makefile to select linker fiel for m2s-creative/polarfire boards.
...
Modified the flash command implmentation to correct the printing of fiel sizes/name
added soem delay().
tested on both m2s-creative-brd and poalrfire-eval board.
...
There are two demo programs available

	1. rtos-demo  : This demo runs the basic rtos blinky demo as well as task  functions. refer to the source code 
	2. rtos-cli-demo : this shows the generic cli demo fo rtos.
...
Made option of using a `BUILD` option compile time. This is to generate debug/reales codes
...
Added the automatic selection of linker fiel for m2s-creative and polarfire boards.

if m2s-creive-baord is selected, by default the esrma.ld will be used. ram.ld will be used for poalrfire boards.

use the customer linker option are to override this .
...
# use miv-rv32-esram.ld when using the ymodem,  for the m2s-creative board 
#LDFILE      = boards/$(BOARD)/platform_config/linker/miv-rv32-esram.ld
#LDFILE      = boards/$(BOARD)/platform_config/linker/miv-rv32-ram.ld
```
