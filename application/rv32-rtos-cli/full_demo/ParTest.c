...
#else
	static const gpio_id_t ucLEDs[ partstNUM_LEDS ] = { GPIO_0, GPIO_1 };
#endif
...
long x;
    ...
    {
    	GPIO_config( &g_gpio_out,ucLEDs[ x ], GPIO_OUTPUT_MODE );
...
void vParTestSetLED( portBASE_TYPE uxLED, portBASE_TYPE xValue )
{
	if( uxLED < partstNUM_LEDS )
	{
		taskENTER_CRITICAL();
		{
			GPIO_set_output( &g_gpio_out,ucLEDs[ uxLED ], xValue );
