...
set hwPlatform [string toupper [lindex $argv 3]]
...
#set hwPlatform "M2S_CREATIVE"
#set hwFamily "SmartFusion2"
...
if {[file exists $projectDir] == 1} then {
	...
	new_project \
		...
		-package $dieSize \
		-speed $dieSpeed \
		-die_voltage $dieVoltage \
		-part_range $tempGrade \
		-adv_options "IO_DEFT_STD:$defIOstd" \
...
if {$hwPlatform eq "M2S_CREATIVE"} {
	source $scriptDir/import/build_smartdesign/M2S_CREATIVE/$sdBuildScript
} else {
