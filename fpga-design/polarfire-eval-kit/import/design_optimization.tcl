...
if {$config in {"CFG1" "CFG2" "CFG3" "DGC1" "DGC3" "DGC4"}} then {
	import_files -io_pdc $scriptDir/import/constraints/$hwPlatform/io/io_constraints.pdc
	import_files -io_pdc $scriptDir/import/constraints/$hwPlatform/io/uart_constraints.pdc
	import_files -sdc $scriptDir/import/constraints/$hwPlatform/timing/io_jtag_constraints.sdc

	if {$config in { "CFG1" "CFG2" "CFG3"}} {
		import_files -io_pdc $scriptDir/import/constraints/$hwPlatform/io/spi_constraints.pdc
...
} elseif {$config in {"CFG1" "CFG2" "CFG3"}} {
	organize_tool_files -tool {PLACEROUTE} \
		-file $projectDir/constraint/io/io_constraints.pdc \
        -file $projectDir/constraint/io/uart_constraints.pdc \
