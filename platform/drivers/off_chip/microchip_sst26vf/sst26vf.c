...
#define FLASH_SPI_INSTANCE   &g_spi_0
...
spi_flash_status_t FLASH_init( void )
{
	...
	SPI_init(&g_spi_0, CORESPI_BASE_ADDR,32 );
	SPI_configure_master_mode( &g_spi_0 );
	/* set the spi clock divisions */
	HAL_set_8bit_reg(CORESPI_BASE_ADDR, CLK_DIV, 8 );
	SPI_enable(&g_spi_0);
	return 0;
...
)
{
	...
	*manufacturer_id = 0;
	*device_id = 0;
	SPI_set_slave_select(FLASH_SPI_INSTANCE, SPI_SLAVE_0);
	SPI_transfer_block( FLASH_SPI_INSTANCE, &read_device_id_cmd, 1, read_buffer, sizeof(read_buffer) );
	SPI_clear_slave_select(FLASH_SPI_INSTANCE, SPI_SLAVE_0 );
...
)
{
	...
	SPI_set_slave_select(FLASH_SPI_INSTANCE, SPI_SLAVE_0);
	wait_ready();
	SPI_transfer_block( FLASH_SPI_INSTANCE, cmd_buffer, 4, rx_buffer, size_in_bytes );
	wait_ready();
	SPI_clear_slave_select(FLASH_SPI_INSTANCE, SPI_SLAVE_0 );
	return 0;
...
spi_flash_status_t FLASH_global_unprotect( void )
{
	...
	SPI_set_slave_select(FLASH_SPI_INSTANCE, SPI_SLAVE_0);
	//wait_ready();
	SPI_transfer_block( FLASH_SPI_INSTANCE, cmd_buffer, 2, 0, 0 );
	//wait_ready();
	SPI_clear_slave_select(FLASH_SPI_INSTANCE, SPI_SLAVE_0 );
	return 0;
...
)
{
	...
	SPI_transfer_block( FLASH_SPI_INSTANCE, tx_buffer, transfer_size, 0, 0 );
...
static void write_enable()
{
	...
	//wait_ready();
	SPI_transfer_block( FLASH_SPI_INSTANCE, &cmd_buffer, 1, 0, 0 );
	//wait_ready();
	SPI_clear_slave_select( FLASH_SPI_INSTANCE,SPI_SLAVE_0);
}

spi_flash_status_t FLASH_write
(
		uint32_t address,
...
static void wait_ready( void )
{
	...
	do {
		...
		SPI_transfer_block( FLASH_SPI_INSTANCE, &command, sizeof(command), &ready_bit, sizeof(ready_bit) );
...
)
{
	...
	cmd_buffer[3] = (uint8_t)(address & 0xFF);
	wait_ready();
	SPI_transfer_block(FLASH_SPI_INSTANCE, cmd_buffer,4, 0, 0 );
	wait_ready();
