...
// this section is , if using CORESPI
...
/*
  * Memory configuration is with 254 * 64KB blocks and 32 * 4K blocks.
  * For erasing, one sector size is 64KB and need to find how many secotros need to erase form the total data length,
  *  Similarly, the offset address for the sector also needs to eb calcualted
...
spi_flash_status_t FLASH_get_status( void )
{
   ...
   SPI_transfer_block( SPI_FLASH_INSTANCE, &command,sizeof(uint8_t), &status, sizeof(status) );
